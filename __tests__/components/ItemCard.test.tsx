...
describe('formatDescription', () => {
  ...
  it('creates links for URLs', () => {
    ...
    const links = findElements<LinkProps>(result, 'a')
  ...
  it('creates buttons for dice notation', () => {
    ...
    const buttons = findElements<ButtonProps>(result, 'button')
...
function flatten(node: React.ReactNode): string {
  ...
  if (typeof node === 'object' && 'props' in node) {
    return flatten((node as React.ReactElement<{ children?: React.ReactNode }>).props.children)
  }
  return ''
}

/** Recursively find all React elements of a given type in the tree. */
function findElements<P>(node: React.ReactNode, type: string): Array<React.ReactElement<P>> {
  const results: Array<React.ReactElement<P>> = []
  function walk(n: React.ReactNode) {
    if (!n || typeof n === 'string' || typeof n === 'number') return
    if (Array.isArray(n)) { n.forEach(walk); return }
    if (typeof n === 'object' && 'type' in n) {
      const el = n as React.ReactElement<{ children?: React.ReactNode } & Record<string, unknown>>
      if (el.type === type) results.push(el as unknown as React.ReactElement<P>)
